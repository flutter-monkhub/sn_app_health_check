  ...
  for (const url of APIS) {
    try {
      ...
      if (res.status >= 200 && res.status < 300) {
        console.log(`OK: ${url}`);
        results.push({ url, status: "ok", code: res.status });
      } else {
        console.log(`FAIL: ${url} -> ${res.status}`);
        await sendAlert(url, `HTTP ${res.status}`);
        results.push({ url, status: "fail", code: res.status });
      }
    } catch (err) {
      console.log(`DOWN: ${url} -> ${err.message}`);
      await sendAlert(url, err.message);
      results.push({ url, status: "down", error: err.message });
    }
  }
...
module.exports = async function handler(req, res) {
  // Only allow GET (from cron) or POST (manual trigger)
